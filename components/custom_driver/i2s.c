...
esp_err_t i2s_custom_init_dma_tx_queues(i2s_port_t i2s_num, uint8_t *data, size_t size, size_t *written) {
	...
	if ((data != NULL) && (written != NULL) && (size != 0)) {
		size_t offset = 0;
		size_t maxDmaBufBytes = p_i2s_obj[i2s_num]->dma_buf_len * p_i2s_obj[i2s_num]->bytes_per_sample * p_i2s_obj[i2s_num]->channel_num;
...
//				ESP_LOGW(I2S_TAG, "wrote %d", maxDmaBufBytes);
...
//				ESP_LOGW(I2S_TAG, "wrote %d", tmpSize);
...
//    i2s_custom_start(i2s_num);	// don't start just yet, we want to fill dma buffer first
//    return ESP_OK;

    // ensure all DMA buffers are available right after initialization
	// ENSURE i2s_custom_start() isn't called before i2s_write has filled at least one buffer
...
esp_err_t i2s_custom_set_sample_rates(i2s_port_t i2s_num, uint32_t rate)
{
    ...
    return i2s_custom_set_clk(i2s_num, rate, p_i2s_obj[i2s_num]->bits_per_sample, p_i2s_obj[i2s_num]->channel_num);
}
...
esp_err_t i2s_custom_set_pdm_rx_down_sample(i2s_port_t i2s_num, i2s_pdm_dsr_t dsr)
{
    ...
    return i2s_custom_set_clk(i2s_num, p_i2s_obj[i2s_num]->sample_rate, p_i2s_obj[i2s_num]->bits_per_sample, p_i2s_obj[i2s_num]->channel_num);
}
#endif

static esp_err_t i2s_custom_check_cfg_static(i2s_port_t i2s_num, const i2s_config_t *cfg)
{
#if SOC_I2S_SUPPORTS_PDM
...
static esp_err_t i2s_param_config(i2s_port_t i2s_num, const i2s_config_t *i2s_config)
{
    ...
    I2S_CHECK((i2s_custom_check_cfg_static(i2s_num, i2s_config) == ESP_OK), "param check error", ESP_ERR_INVALID_ARG);
...
esp_err_t i2s_custom_zero_dma_buffer(i2s_port_t i2s_num)
{
    ...
    if (p_i2s_obj[i2s_num]->tx && p_i2s_obj[i2s_num]->tx->buf != NULL && p_i2s_obj[i2s_num]->tx->buf_size != 0) {
        ...
        if (bytes_left) {
            ...
            i2s_custom_write(i2s_num, (void *)&zero_bytes, bytes_left, &bytes_written, portMAX_DELAY);
        }
        for (int i = 0; i < p_i2s_obj[i2s_num]->dma_buf_count; i++) {
...
esp_err_t i2s_custom_driver_install(i2s_port_t i2s_num, const i2s_config_t *i2s_config, int queue_size, void* i2s_queue)
{
    ...
    if (err != ESP_OK) {
        ...
        //set clock and start
        return i2s_custom_set_clk(i2s_num, i2s_config->sample_rate, i2s_config->bits_per_sample, p_i2s_obj[i2s_num]->channel_num);
    }
...
esp_err_t i2s_custom_write(i2s_port_t i2s_num, const void *src, size_t size, size_t *bytes_written, TickType_t ticks_to_wait)
{
    ...
    *bytes_written = 0;
    ...
    while (size > 0) {
//        for (int i = 0; i < p_i2s_obj[i2s_num]->dma_buf_count; i++) {
//        	ESP_LOGI(I2S_TAG,"%d: EOF %d",  i, p_i2s_obj[i2s_num]->tx->desc[i]->eof);
//        }
