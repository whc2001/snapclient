                    ...
                    case 7: {
                      ...
                      if (typedMsgCurrentPos >= base_message_rx.size) {
                        ...
                        // these we can get server now
                        {
                          ...
                          if (xSemaphoreTake(timeSyncSemaphoreHandle, 0) ==
                              pdTRUE) {
                            ESP_LOGW(TAG,
                                     "couldn't take "
 ...
 *
 */
static void http_get_task_backup(void *pvParameters) {
  struct sockaddr_in servaddr;
  char *start;
  int sock = -1;
  ...
  while (1) {
    ...
    for (;;) {
      ...
      if (result > 0) {
        ...
        result = base_message_deserialize(&base_message,
                                          base_message_serialized, size);
        if (result) {
        ...
        if (lastTypedMsgSize < base_message.size) {
          ...
          //				  if (typedMsg == NULL)
          {
            ...
            typedMsg = (char *)heap_caps_realloc(typedMsg, base_message.size,
                                                 MALLOC_CAP_8BIT);
            ...
            //            MALLOC_CAP_8BIT);
            if (typedMsg == NULL) {
              ESP_LOGE(TAG, "Couldn't get memory for typed message %d, %d, %d",
                       base_message.size,
                       heap_caps_get_free_size(MALLOC_CAP_8BIT),
                       heap_caps_get_largest_free_block(MALLOC_CAP_8BIT));
        ...
        switch (base_message.type) {
          case SNAPCAST_MESSAGE_CODEC_HEADER:
            ...
            if (strcmp(codec_header_message.codec, "opus") == 0) {
              ...
              if (t_flac_decoder_task == NULL) {
                xTaskCreatePinnedToCore(&flac_decoder_task, "flac_decoder_task",
              ...
              ESP_LOGI(TAG, "%s sampleformat: %d:%d:%d",
                       codec_header_message.codec, scSet.sr, scSet.bits,
                       scSet.ch);
          ...
          case SNAPCAST_MESSAGE_WIRE_CHUNK: {
            ...
            switch (codec) {
              case OPUS: {
                ...
                if (audio == NULL) {
                  ...
                  audio = (int16_t *)heap_caps_malloc(
                      ...
                      MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);  // 960*2: 20ms,
                                                             // 960*1: 10ms
#else
                  audio = (int16_t *)malloc(
                      frameSize * scSet.ch *
                      (scSet.bits / 8));  // 960*2: 20ms, 960*1: 10ms
#endif
                }
                ...
                if (audio == NULL) {
                  ...
                  while ((frame_size = opus_decode(
                    ...
                    audio = (int16_t *)heap_caps_realloc(
                        audio, pcm_size * scSet.ch * (scSet.bits / 8),
                        MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);  // 2 channels +
                                                               // 2 Byte per
                                                               // sample ==
                                                               // int32_t
#else
                  ...
                  if (frame_size < 0) {
                    ...
                    pcm_chunk_message.size =
                        frame_size * scSet.ch * (scSet.bits / 8);
                    pcm_chunk_message.payload = (char *)audio;
                    pcm_chunk_message.timestamp = timestamp;

                    scSet.chkDur_ms = (1000UL * pcm_chunk_message.size) /
                                      (uint32_t)(scSet.ch * (scSet.bits / 8)) /
                                      scSet.sr;
                    if (player_send_snapcast_setting(&scSet) != pdPASS) {
                      ESP_LOGE(TAG,
              ...
              case PCM: {
                ...
                pcm_chunk_message.timestamp = timestamp;
                pcm_chunk_message.payload = wire_chunk_message.payload;
          ...
          case SNAPCAST_MESSAGE_TIME:
            ...
            // now
            {
              ...
              if (diff.tv_sec > 60) {
                ESP_LOGW(TAG,
                         "Last time sync older than a minute. "
                         "Clearing time buffer");
              ...
              if (xSemaphoreTake(timeSyncSemaphoreHandle, 0) == pdTRUE) {
                ESP_LOGW(TAG, "couldn't take timeSyncSemaphoreHandle");
              }
      ...
      if (received_header == true) {
        if (xSemaphoreTake(timeSyncSemaphoreHandle, 0) == pdTRUE) {
          ...
          result = time_message_serialize(
              &time_message, time_message_serialized, TIME_MESSAGE_SIZE);
          if (result) {
